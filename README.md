...
- [EruditePay](https://bridge.eruditepay.com) - 352 x402-payable blockchain analytics endpoints across Base, Tron, BTC, XRP, and Kaspa. Token prices, whale tracking, DeFi analytics, gas forecasting, NFT data, and security scanning. 168 MCP tools. Pay-per-call with USDC (Base) and USDT (Tron). Self-hosted facilitator with dual-network settlement. ([Website](https://eruditepay.com)) | ([Discovery](https://bridge.eruditepay.com/.well-known/x402)) | ([Agent Card](https://bridge.eruditepay.com/.well-known/agent-card.json))
